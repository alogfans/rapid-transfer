...
// Buffer info structure for RPC exchange
struct BufferInfo {
...
class SessionManager {
   public:
    ...
    // Set callback for receiving notification messages from remote peers
    void setNotificationCallback(OnNotificationCallback callback);

    // RPC handler for notification messages (need to be public for coro_rpc)
    int handleNotification(const std::string& peer_name, int task_id,
                           const std::string& message);

    // Buffer info management for e2e testing
    // Set local buffer info (called by receiver)
    void setBufferInfo(const BufferInfo& info);

    // RPC handler to get buffer info (called by sender)
   ...
   private:
    ...
    // Map from original peer address to generated session name (e.g.,
    // "localhost:12359" -> "server/0")
