...
struct QueueEntry {
    QueueEntry(size_t fragment_capacity = 4000)
        : offset_(0), fragment_id_(0), fragment_capacity_(fragment_capacity) {}

    bool hasRemainingFragment() { return !buffer_list_.empty(); }

    Buffer popFragment() {
        if (buffer_list_.empty()) return Buffer{nullptr, 0};
    ...
    std::pair<uint64_t, uint64_t> push(const std::vector<Buffer> &buffer_list) {
        ...
        for (auto &entry : buffer_list) {
            ...
            fragment_id_ +=
                (entry.length + fragment_capacity_ - 1) / fragment_capacity_;
        }
