...
struct SessionIdManager {
   public:
    ...
    std::string getEndPointByReceiver(int sid) {
        ...
        if (sid_to_sender_.count(sid)) return sid_to_sender_[sid];
        return "";
    }

    int getSidBySender(const std::string &receiver) {
        RWSpinlock::ReadGuard guard(session_lock_);
        if (receiver_to_sid_.count(receiver)) return receiver_to_sid_[receiver];
        return -1;
    }

    int getSidByReceiver(const std::string &sender) {
        RWSpinlock::ReadGuard guard(session_lock_);
        if (sender_to_sid_.count(sender)) return sender_to_sid_[sender];
        return -1;
    }
