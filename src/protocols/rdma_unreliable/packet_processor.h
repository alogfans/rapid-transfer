...
namespace rapid
{
    ...
    public:
        ...
        SessionIdManager &sessionIdManager() { return protocol_.session_id_manager_; }

    public:
        bool connected(const std::string &peer_name);

        int issuePackets(const std::string &peer_name, RequestType type, const std::vector<Buffer> &buffer_list, uint32_t &next_sn);
