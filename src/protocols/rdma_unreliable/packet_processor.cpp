...
namespace rapid
{
    ...
    int PacketProcessor::construct()
    {
        ...
        worker_list_.emplace_back(std::thread(std::bind(&PacketProcessor::worker, this)));
        return 0;
    }
    ...
    bool PacketProcessor::connected(const std::string &peer_name)
    {
        ...
        if (!sessions_.count(peer_name))
            return false;
        auto &session = sessions_[peer_name];
        return session.status == SESSION_OK;
    }

    int PacketProcessor::issuePackets(const std::string &peer_name, RequestType type,
    ...
    uint32_t PacketProcessor::nextPacketSN(const std::string &peer_name, RequestType type)
    {
        RWSpinlock::ReadGuard guard(sessions_lock_);
        if (!sessions_.count(peer_name))
            return UINT32_MAX;
    ...
    int PacketProcessor::prepareConnection(const std::string &peer_name, Attributes &local)
    {
        ...
        local["session"] = std::to_string(protocol_.session_id_manager_.allocateSidByReceiver(peer_name));
        return 0;
    }

    int PacketProcessor::setupConnection(const std::string &peer_name, const Attributes &peer)
    {
        auto endpoint = endpoint_store_.getOrCreateEndpoint(peer_name);
        if (!endpoint)
            return -1;
