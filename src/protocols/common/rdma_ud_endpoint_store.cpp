...
int RdmaUDEndPointStore::deconstruct() {
    ...
    for (size_t i = 0; i < qp_list_.size(); ++i) {
        ...
        //     PLOG(WARNING) << "Outstanding work requests found, CQ will not be
        //     generated";
...
std::shared_ptr<RdmaUDEndPoint> RdmaUDEndPointStore::getOrCreateEndpoint(
    ...
    if (endpoint_map_.count(peer_nic_path)) return endpoint_map_[peer_nic_path];
...
int RdmaUDEndPointStore::postSendRequest(
    const std::vector<Request *> &request_list, ibv_ah *ah, uint32_t remote_qpn,
    int qp_index) {
...
int RdmaUDEndPointStore::postReceiveRequest(
    const std::vector<Request *> &request_list, int qp_index) {
