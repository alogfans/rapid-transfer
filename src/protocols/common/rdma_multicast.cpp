...
int RdmaMulticastContext::construct(const std::string &local_addr,
    ...
    for (size_t i = 0; i < config_.num_qp_per_endpoint; ++i) {
        ...
        ret = rdma_resolve_addr(connection.cm_id, local_addr_, multicast_addr_,
                                2000);
        if (ret) {
...
int RdmaMulticastContext::registerMemoryRegion(void *addr, size_t length,
                                               int access) {
...
int RdmaMulticastContext::unregisterMemoryRegion(void *addr) {
    for (size_t i = 0; i < connections_.size(); ++i) {
        ...
        do {
            has_removed = false;
            for (auto iter = memory_regions.begin();
                 iter != memory_regions.end(); ++iter) {
                if ((*iter)->addr <= addr &&
...
std::pair<uint32_t, uint32_t> RdmaMulticastContext::key(void *addr,
                                                        int conn_index) {
    if (conn_index < 0 || conn_index >= (int)connections_.size()) return {0, 0};

    auto &memory_regions = connections_[conn_index].memory_regions;
    for (auto iter = memory_regions.begin(); iter != memory_regions.end();
         ++iter)
        if ((*iter)->addr <= addr &&
            addr < (char *)((*iter)->addr) + (*iter)->length)
...
int RdmaMulticastContext::postReceiveRequest(
    const std::vector<Request *> &request_list, int conn_index) {
    if (conn_index < 0 || conn_index >= (int)connections_.size()) return -1;
...
int RdmaMulticastContext::onAddressResolved(void *context) {
    ...
    if (on_init_qp_hook_) ret = on_init_qp_hook_(connection->cm_id->qp);
    if (ret) return ret;
...
int RdmaMulticastContext::onMulticastJoin(void *context,
                                          struct rdma_ud_param *param) {
    Connection *connection = (Connection *)context;
    char buf[40];
    inet_ntop(AF_INET6, param->ah_attr.grh.dgid.raw, buf, 40);
    LOG(INFO) << "Joined: dgid " << buf << " mlid " << param->ah_attr.dlid
              << " sl " << param->ah_attr.sl;
