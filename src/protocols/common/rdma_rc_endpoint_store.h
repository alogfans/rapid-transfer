...
class RdmaRCEndPointStore {
   ...
   public:
    ...
    std::shared_ptr<RdmaRCEndPoint> getOrCreateEndpoint(
        ...
        int ret = endpoint->construct(
            ...
            config.num_qp_per_endpoint, config.max_sge_per_wr,
            config.max_wr_per_qp, config.max_inline_bytes);
   ...
   private:
    ...
    std::unordered_map<std::string, std::shared_ptr<RdmaRCEndPoint>>
        endpoint_map_;
