...
#include "../rdma_reliable/rdma_reliable_protocol.h"
...
namespace rapid
{
    ...
    int RdmaEndPoint::postRequest(RequestType type, const std::vector<Request *> &request_list)
    {
        int qp_index = 0;
        int wr_count = std::min(max_wr_depth_ - wr_depth_list_[qp_index], (int)request_list.size());
        if (wr_count == 0)
            return 0;
        ...
        if (type == RequestType::SEND)
        {
            ...
            for (int i = 0; i < wr_count; ++i)
            {
                ...
                request->qp_depth = &wr_depth_list_[qp_index];
            }
            ...
            if (rc)
            {
                ...
                while (bad_wr)
                {
                    ...
                    request_list[i]->task->failed_packets++;
