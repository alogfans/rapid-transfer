...
namespace rapid {
    ...
    public:
        ...
        int construct(const std::string &local_addr, const std::string &multicast_addr, size_t num_connections);
    ...
    public:
        int postSendRequest(const std::vector<Request *> &request_list, int conn_index = 0);

        int postReceiveRequest(const std::vector<Request *> &request_list, int conn_index = 0);

        int poll(int num_entries, ibv_wc *wc, int conn_index = 0);

        void setInitQpHook(std::function<int(ibv_qp *)> on_init_qp_hook) { on_init_qp_hook_ = on_init_qp_hook; }
